...
#   账号1 → 无后缀: EUSERV_EMAIL / EUSERV_PASSWORD / IMAP_SERV / EMAIL_PIN / EMAIL_PASS
#   账号2 → 后缀2:  EUSERV_EMAIL2 / EUSERV_PASSWORD2 / IMAP_SERV2 / EMAIL_PIN2 / EMAIL_PASS2
#   账号N → 后缀N
# -------------------------------------------------------
_suffix() {
    # 账号1返回空字符串，账号2+返回数字
...
configure_env() {
    ...
    accounts=()   # 每个元素: "email|password|imap|pin_email|pin_pass"
    account_index=1
    ...
    while true; do
        ...
        read -p "  接收验证码PIN的邮箱: " acc_pin_email
        read -sp "  接收PIN邮箱的应用专用密码: " acc_pin_pass
        echo ""

        if [[ -z "$acc_email" || -z "$acc_password" || -z "$acc_pin_email" || -z "$acc_pin_pass" ]]; then
            print_warning "必填项不能为空，请重新输入"
            continue
        fi

        accounts+=("${acc_email}|${acc_password}|${acc_imap}|${acc_pin_email}|${acc_pin_pass}")
...
_delete_account_fields() {
    ...
    local sfx
    [[ "$idx" -eq 1 ]] && sfx="" || sfx="$idx"
    for prefix in EUSERV_EMAIL EUSERV_PASSWORD IMAP_SERV EMAIL_PIN EMAIL_PASS; do
...
_add_account() {
    ...
    read -p "  接收验证码PIN的邮箱: " new_pin_email
    read -sp "  接收PIN邮箱的应用专用密码: " new_pin_pass; echo ""

    if [[ -z "$new_email" || -z "$new_pass" || -z "$new_pin_email" || -z "$new_pin_pass" ]]; then
    ...
    local sfx; [[ "$new_idx" -eq 1 ]] && sfx="" || sfx="$new_idx"
    {
        ...
        echo "EMAIL_PIN${sfx}=${new_pin_email}"
...
_edit_account() {
    ...
    echo "  收件邮箱: $(_get_field EMAIL_PIN $edit_idx)"
    echo "（直接回车表示保持不变）"
    echo ""
    ...
    read -p "  新的收件邮箱: " v; [[ -n "$v" ]] && _set_field EMAIL_PIN "$edit_idx" "$v"
